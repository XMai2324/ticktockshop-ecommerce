...
@php
    ...
    $toArray = function ($raw) {
        if (!$raw) return [];
        if (is_array($raw)) return array_values(array_filter($raw, fn($v)=>is_string($v) && trim($v)!=''));
        $decoded = json_decode($raw, true);
        if (json_last_error()===JSON_ERROR_NONE && is_array($decoded)) {
            return array_values(array_filter($decoded, fn($v)=>is_string($v) && trim($v)!=''));
        }
        return array_values(array_filter(preg_split('/\s*[,;|]\s*/', (string)$raw) ?: [], fn($v)=>is_string($v) && trim($v)!=''));
    };
    $fileNames = $toArray($product->images ?? null);

    $thumbUrls = collect($fileNames)->map(function($img) use ($folder) {
        $img = ltrim($img, '/');
        if (preg_match('~^https?://~i', $img)) return $img;   // URL tuyệt đối
        return asset('storage/' . $folder . '/' . $img);       // tên file -> ghép folder
    })->unique()->reject(fn($u) => $u === $imageUrl);
@endphp
          ...
          <div class="product-page-right-content">
            @foreach($related as $r)
              @php
                ...
                if ($rCat==='nam') $rf='Watch/Watch_nam';
                elseif ($rCat==='cap-doi') $rf='Watch/Watch_cap';
              @endphp
              <div class="product-page-right-content-item">
                <a href="{{ route('product.detail', ['product' => $r->slug]) }}" class="product-card">
                  <img src="{{ asset('storage/' . $rf . '/' . $r->image) }}" alt="{{ $r->name }}">
